...
class Frequency():
    ...
    def get_frequency(self, paragraph):
        ...
        tokens = self.tokenizer.convert_ids_to_tokens(input_paragraph['input_ids'])
        ...
        result = modified_text.replace('[CLS]','').replace('[SEP]','.').replace('[sep]','')

        return result, top_four_words
